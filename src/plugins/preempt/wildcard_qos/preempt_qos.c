	...
	job_record_t *preemptee, job_record_t *preemptor)
{
    ...
    char *name_ee = qos_ee->name;
    char *name_or = qos_or->name;

    // check if preemptor and preemptee are lcls jobs
    const char *colon_pos_or = xstrchr(name_or, ':');
    const char *colon_pos_ee = xstrchr(name_ee, ':');
    if (colon_pos_or != NULL && colon_pos_ee != NULL) {
        if (xstrncmp(name_or, "lcls", colon_pos_or) == 0 &&
            xstrncmp(name_ee, "lcls", colon_pos_ee) == 0) {
            // extract caret (^) location for experiment name
            //const char *caret_pos_or = xstrchr(name_or, '^');
            //const char *caret_pos_ee = xstrchr(name_ee, '^');
            //if (caret_pos_or == NULL or caret_pos_ee == NULL) {
                // invalid name
            //}
            ...
            regex_t online_reg;
            regex_t offline_reg;
            const char* normal_pat = "lcls:[[:alnum:]]+\\^normal@[[:alnum:]]+";
            const char* online_pat = "lcls:[[:alnum:]]+\\^online@[[:alnum:]]+";
            const char* offline_pat = "lcls:[[:alnum:]]+\\^offline@[[:alnum:]]+";
            if (regcomp(&normal_reg, normal_pat, REG_EXTENDED != 0))
                printf("Failed to compile normal regex");
            if (regcomp(&online_reg, online_pat, REG_EXTENDED != 0))
                printf("Failed to compile normal regex");
            if (regcomp(&offline_reg, offline_pat, REG_EXTENDED != 0))
                printf("Failed to compile normal regex");

            // preemption logic 
            if (regexec(&online_reg, name_ee, 0, NULL, 0) == 0) {
                return false;
            }
            if (regexec(&offline_pat, name_ee, 0, NULL, 0) == 0) {
                // if preemptor is not online, return false
                if (regexec(&online_pat, name_or, 0, NULL, 0) != 0)
                    return false;
            }
            /*
            if (regexec(&normal_pat, name_ee, 0, NULL, 0) == 0) {
                // if same exp, yes, otherwise no
                if ()
                {
            ...
            */        
            ...
            return origin(preempt_p_preemptable);
        }
    } else {
        return origin_preempt_p_preemptable();
    }
 
    // end custom wildcard qos

    return true;
}

int origin_preempt_p_preemptable(
	job_record_t *preemptee, job_record_t *preemptor)
{
    if (ret != 0)
        return false;
